...
function Cards({ users }) {
  const { getID, editContact, deleteContact, filterContacts,search } = useCrudContext();

  const noResults = search.trim() !== '' && filterContacts.length === 0 && users.length > 0;
    ...
    <div className="block md:hidden space-y-8 cards">
      {
        noResults ?
          (
            <p className='text-accent-warm text-shadow-md text-shadow-accent-warm'>No se encontraron Contactos</p>
          )
          :
          (filterContacts.length > 0 ? filterContacts : users).map((user) => (
